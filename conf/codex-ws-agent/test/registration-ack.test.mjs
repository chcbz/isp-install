...
test('agent runtime wires send, inbound control, timeout, and disconnect to the observer', () => {
  ...
  assert.match(source, /const registrationResult = state\?\.registration\.observe\(parsed\)/)
  assert.match(source, /if \(registrationResult === 'registered' && profile\.managedGeneration\s*&& managedHostModule\?\.managedRegistration\(parsed, profile, PROCESS_RUNTIME_INSTANCE_ID\)\)/)
